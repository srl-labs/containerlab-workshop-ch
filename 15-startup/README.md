...
We start by deploying a lab defined in the `startup.clab.yml` topology file. The lab consists of two nodes: `srl` (Nokia SR Linux) and `sros` (Nokia SR-OS). Both nodes are configured with a startup configuration file that resides in the same directory as the topology file.
...
The startup configuration files - [srl.cfg](srl.cfg) and [sros.cfg](sros.cfg) - configure the interfaces, IP addressing, loopbacks and BGP peering between SR Linux and SR-OS nodes respectively.

In particular, the `srl` node is configured to announce its loopback address `10.10.10.1/32` towards the `sros` node and the `sros` node is configured to announce its loopback address `10.10.10.2/32` towards the `srl` node.
...
Now, let's connect to the `clab-startup-sros` node and make sure that it can reach the loopback address announced by the `srl` node.

```bash
ssh clab-startup-sros
```
